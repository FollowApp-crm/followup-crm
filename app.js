...
(function(){
  ...
  function applyTheme(t){
    ...
    $('#themeToggle').textContent = (t==='light' ? '🌙 Dark' : '☀️ Light');
  }
  ...
  function makeModalFromCard(cardId, modalId){
    ...
    modal.addEventListener('click', (e)=>{ if(e.target===modal) { modal.classList.remove('open'); modal.style.display='none'; document.body.classList.remove('modal-open'); }});
    trapFocus(modal);
    return modal;
  }
  ...
  function parseLeadBlob(text){
    ...
    // Remove ISO dates to avoid capturing times as phones
    ...
    for(const cand of idCandidates){
      if (/^\d{8}$/.test(cand)) continue;      // likely a date without dashes (YYYYMMDD)
      if (/^\d{10,11}$/.test(cand)) continue;  // likely phone leftovers
      leadId = cand; break;
    }
  ...
  /* ========= Contact links ========= */
  const CALL_LINK_MODE = 'tel';  // or 'callto'
  const EMAIL_MODE     = 'gmail'; // or 'mailto'
  ...
  // Global copy buttons (table + agenda)
  ...
  let showMode = 'all'; // 'all' | 'open' | 'done'
  ...
  function renderTask(t){
    ...
    div.innerHTML = `
      ...
      <div class="tiny mono" style="display:flex; align-items:center; gap:6px">
        <span>${t.date}</span>
        ${timeChip}
        <button class="btn-icon" data-bell="${t.id}" title="${bellTitle}" aria-label="${bellTitle}">${bell}</button>
  ...
  /* ========= Notifications boot ========= */
  function initNotificationsUI_and_start(){
    initNotificationsUI();
  ...
  function bootstrap() {
    ...
    $('#closeAdd')?.addEventListener('click', () => { hideBulkImport(); closeModal('addModal', 'addCard'); });
